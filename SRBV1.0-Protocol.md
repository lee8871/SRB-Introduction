...
每次访问都将数据写入节点，并读取从机采集到的数据。

#### 机器人·Robotic
SRB是专门为了机器人场合使用的总线协议，特别说明，这里的机器人更倾向于非工业机器人，例如服务机器人。
这些机器人工作在非结构化环境[^1]，需要大量的传感器来检测环境，而对精度要求不高。
SRB让机器人能够连接更多数量的传感器，且接线方便，改造容易。
![](http://assets.processon.com/chart_image/5a66d2b1e4b0c0905255d0aa.png?_=1541310988470)

[^1]:非结构化环境：环境中的物体没有事先进行测量，并且会随时变化。例如家庭中，水杯的位置经常改变，桌子常常被撞歪。相对而言，工厂中的工作台，轨道，夹具，传送带等设备都被固定在地面或CNC中，机器人预先知道这些设备的位置，这种环境为结构化环境。

#### 简单·Simple
##### 易学
SRB遵守`实现一种功能有且最好仅有一种方法`的设计理念。
事实上，使用SRB将功能全部压缩到了`数据传输`和`配置传输`两种访问模式，前者用于进行双向通信，传输实时信息。后者用于配置，传输运行参数，SRB运行参数断电不消失。
##### 好用
SRB产品非常易于接线，
