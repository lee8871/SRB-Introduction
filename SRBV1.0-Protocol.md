...
![IDC端子刺破排线表皮连线接线快，一条排线可以连接任意多个SRB设备](./Image/IDC端子.png)</br>
...
**Bfc.busy**|1/8|如果节点收到了数据，但是没有空间储存数据，则返回1, 否则返回0。只有配置簇可能返回busy。
**Bfc.event**|1/8|~~当节点存在事件时，节点可以在返回包中将这一位标记为1，主机会选择进行数据访问以获得事件信息。~~保留,这部分功能转移到应用层,这一位保留.
**Bfc.length**|5/8| 包的数据负载Data部分的长度。长度是0到31的数字。
